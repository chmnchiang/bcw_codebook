...
\lstinputlisting{../codes/vimrc/.vimrc}
...
int len;
int cc[30];
int rk[MAX],sa[MAX],tsa[MAX],sm[MAX];
pii tp[MAX];
int he[MAX];
//
void meteor()
{
	...
	int j,k=0;
	for(int i=0;i<len;he[rk[i++]]=k)
	{
		for(k?k--:0,j=sa[rk[i]-1];ip[i+k]==ip[j+k];k++);
	}
}
//
int main()
...
\end{lstlisting}
\subsection{Suffix Array Problem}
\begin{lstlisting}
const int MAX = 123123;
