...
int N,V,M,deg[MAXN],val[MAXN],sum[MAXN];
int dp[MAXN];
int ans[MAXN];
vector<pii> E[MAXN];
pii edge[MAXN];
...
void DFS(int u, int f, int tot) {
	...
	for (auto it:E[u]) {
		...
		DFS(v,u,tot);
		dp[u] += dp[v] + 1;
		if (dp[v] % 2 == 0 and (tot - 1 - dp[v]) % 2 == 0) {
			ans[eid] = 1;
		}
	}
}
void solve(vector<int> vec) {
	if (SZ(vec) <= 1) return;
	vector<int> vvv;
	for (auto i:vec) vvv.PB(djs.f(i));
	sort(ALL(vvv));
	vvv.resize(unique(ALL(vvv))-begin(vvv));
...
int main() {
	...
	if (odd >= 2) {
		...
		REP(i,V) ban[i] = 1;
		for (auto v:G.conn[is]) ban[v] = 0;
		REP(i,M) {
			if (ban[edge[i].F]) ans[i] = 0;
		}
	}
