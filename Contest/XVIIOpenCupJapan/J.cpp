...
typedef cc_hash_table<long long,int> umap_t;
#define tree tree12312312312312
//map<vector<int>, int> mp;
umap_t mp;

long long encode(const vector<int> &vec) {
	long long r = 0;
	for (auto i: vec) r = (r << 3) + i + 2;
	return r;
}
vector<int> decode(long long r) {
	vector<int> res;
	REP(i,K) {
		res.PB(r % 8 - 2);
		r /= 8;
...
static void pre() {
	...
	do {
		...
		for (int i=0; i<K; i++) {
			if (!i) {
				res[perm[i]] = (perm[i+1] < perm[i] ? 1 : -1);
			} else if (i == K-1) {
				res[perm[i]] = (perm[i-1] < perm[i] ? 1 : -1);
			} else {
				int s = (perm[i] > perm[i-1] ? 1 : -1) + (perm[i] > perm[i+1] ? 1 : -1);
				res[perm[i]] = s;
			}
		}
		mp[encode(res)] += 2;
...
int calc(vector<int> vec) {
	...
	for (int k=0; k<=K; k++) {
		for (int n=0; n<N; n++) {
			ws360[n][k] = ((vec[k] * D[n] % M) + M ) % M;
		}
	}

	dp[me][0][0] = 1;
	for (int n=0; n<N; n++) {
		memcpy(dp[he], dp[me], sizeof(dp[me]));

		for (int k=0; k<=K; k++) {
			int tm = ws360[n][k];
			for (int m=0; m<M; m++) {
				if (k != K) {
//					int tm = ((m + vec[k]*D[n]) % M + M) % M;
					
					dp[he][k+1][tm] += dp[me][k][m];
					if (dp[he][k+1][tm] >= MD) dp[he][k+1][tm] -= MD;
				}
...
int32_t main() {
	...
	long long ans = 0;
	for (auto p: mp) {
		vector<int> vec = decode(p.F);
//		cout << vec << "(" << p.S << "): " << calc(vec) << endl;


		(ans += 1LL * calc(vec) * p.S) %= MD;
	}
	cout << ans << endl;
