...
vector<A34> E[MAXV];
vector<int> vvv[MAXV];
int fr,bk,que[MAXV],inq[MAXV];

int go(int st) {
	fr = bk = 0;
	REP(i,M) ban[i] = 1;
	REP(i,V) inq[i] = vst[i] = 0;
	que[bk++] = st;
	inq[st] = 1;
	vst[st] = 1;
	while (fr != bk) {
		...
		for (auto it:E[u]) {
			...
			int c = it[1];
			int id = it[2];
			if (ban[id] and c != edge[id].c) continue;
			...
			if (ban[id]) {
//				cout<<"UNBAN "<<u<<" "<<v<<" "<<c<<" "<<id<<endl;
				ban[id] = 0;
				for (auto vv:vvv[id]) {
					if (vst[vv] and !inq[vv]) {
						inq[vv] = 1;
						que[bk++] = vv;
						if (bk >= MAXV) bk -= MAXV;
					}
	...
	REP(i,M) if (ban[i]) return 0;
	return 1;
}
void add_edge(int u, int v, int c, int id) {
//	cout<<"ADD EDGE "<<u<<" "<<v<<" "<<c<<endl;
	E[u].PB((A34){v,c,id});
	E[v].PB((A34){u,!c,id});
}
int main() {
