...
int N,A[MAXN],C[MAXN],sg[MAXN],at[MAXN];

void init(int l, int r, int id) {
	if (l == r) {
		tree[id].s.insert(INF);
		tree[id].v = INF;
		return;
	}
...
void del(int l, int r, int x, int v, int id) {
	if (l == r) {
		tree[id].s.erase(v);
		tree[id].v = *tree[id].s.begin();
		return;
	}
	int m = (l+r)/2, lc = id*2, rc = id*2+1;
	if (x <= m) del(l, m, x, v, lc);
	else del(m+1, r, x, v, rc);
	tree[id].v = min(tree[lc].v, tree[rc].v);
}
void add(int l, int r, int x, int v, int id) {
	if (l == r) {
		tree[id].s.insert(v);
		tree[id].v = *tree[id].s.begin();
		return;
	...
	int m = (l+r)/2, lc = id*2, rc = id*2+1;
	if (x <= m) add(l, m, x, v, lc);
	else add(m+1, r, x, v, rc);
	tree[id].v = min(tree[lc].v, tree[rc].v);
}
