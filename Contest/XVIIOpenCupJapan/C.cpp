...
int main() {
	...
	while (cin>>N>>M) {
		...
		dp[me][0] = 0;
		for (int i=0; i<M; i++) {
			REP1(j,0,W[M]) dp[he][j] = -INF;

			for (int j=0; j<=W[M]; j++) {
				if (dp[me][j] == -INF) continue;
				if (j + A[i] <= W[M]) {
					dp[he][j+A[i]] = max(dp[he][j+A[i]], dp[me][j] + (N-1-i) * (W[i]-j) * A[i] + (W[M]-W[i+1]) * (j+A[i]));
				}
				dp[he][j] = max(dp[he][j], dp[me][j] + (N-1LL-i) * j * A[i] + (W[M]-W[i+1]) * (W[i]-j+A[i]));
			}
	...
	/*	
		REP1(i,0,M) {
			REP1(j,0,W[M]) {
				if (dp[i][j] != -INF) cout<<"DP "<<i<<" "<<j<<" : "<<dp[i][j]<<endl;
			}
		}
*/
