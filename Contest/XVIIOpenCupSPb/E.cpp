...
int judge(pii p, pii pa, pii pb) {
	return (pb.F - pa.F) * (p.S - pa.S) - (p.F - pa.F) * (pb.S - pa.S);
}
...
static void pre() {
	...
	for (int i=2; i<MX; i+=2) {
		way[i] = 0;
		for (int j=0; j<=i-2; j+=2) way[i] += way[j] * way[i-2-j];
		way[i] *= i/2;
	}
}
...
void divide(vector<pii> pts, int mul) {
	int n = SZ(pts);
	if (!n) return;
	sort(ALL(pts));
	for (int i=0; i<n; i++) {
		for (int j=i+1; j<n; j++) {
			...
			int t = way[SZ(lf)] * way[SZ(rf)] * mul;

			if (K < t) {
				ans.PB(pts[i]);
				ans.PB(pts[j]);
				divide(lf, mul * way[SZ(rf)]);
				divide(rf, mul);
				return;
			} else {
				K -= t;
