...
void pre()
{
	...
	for(int i=1; i<MAXN; i++)
		sfor[i] = {A * sfor[i-1].F, A * sfor[i-1].S + B};
	sbak[0] = {1, 0};
	for(int i=1; i<MAXN; i++)
		sbak[i] = {Ar * sbak[i-1].F, Ar * sbak[i-1].S + Br};
...
void print_ans(uint s)
{
	for(int i=0; i<N; i++)
		perm[i] = i;
	uint state = s;
	for(int z=0; z<3; z++)for(int i=0; i<N; i++)
	{
		state = state * A + B;
		int j = (state * (ull)(i+1)) / (1ULL<<32);
		swap(perm[i], perm[j]);
...
uint step(uint x, int s)
{
	if(s >= 0) return sfor[s].F * x + sfor[s].S;
	else return sbak[-s].F * x + sbak[-s].S;
}

puu get_range(ll x, int n)
{
	return {((1LL<<32)*x+n-1)/n, (((1LL<<32)*(x+1)-1)/n) + 1};
}

int change(int r, int x, uint s)
{
	s = step(s, r*N+x+1);
	return (s * (x+1LL))/(1LL<<32);
}

vector<uint> get_valid(int x, int y, int z)
{
	int k = x - y;
	uint alpha = sfor[N-k].F, beta = sfor[N-k].S;
	puu pp = get_range(y, x+1);
	uint l = pp.F, r = pp.S;
	pp = get_range(z, y+1);
	uint lp = pp.F - beta - l * alpha;
	uint rp = pp.S - beta - l * alpha;

	int sz = SZ(kvec[k]);
	int lit = lower_bound(ALL(kvec[k]), puu{lp, 0}) - kvec[k].begin();
	int rit = lower_bound(ALL(kvec[k]), puu{rp, 0}) - kvec[k].begin();
	...
	for(auto s: vs0)
	{
//		uint ss = step(s, x+1+N-k);
//		cout<<ss<<" / "<<l<<"~"<<r<<endl;
//		assert(l <= ss and (ss < r or r == 0));
//		assert(pp.F <= ss and ss < pp.S);
		//if(change(0, x, s) == y) cout<<"MISE"; else cout<<"OK"; cout<<endl;
	}

	return vs0;
}

bool check_seed(uint s)
{
	int x = change(0, N-1, s);
	int y = change(1, x, s);
	int p = pos[N-1];
	while(p > x)
	{
...
uint calc()
{
	...
	for(int x=TH; x<N; x++)
	{
		...
		{
			int y = x - k;
			vector<uint> vs = get_valid(x, y, z);
