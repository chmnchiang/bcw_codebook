...
void calc() {
	...
	REP1(i,1,V) {
		...
		if (val[idx(i)] != -1 or outd[i] != 0) continue;
		res *= (rDFS(i) + 1LL);
		//cout<<"*"<<i<<" "<<rDFS(i)<<endl;
	}
...
void go(int v) {
	...
	if (val[v] != -1) go(v+1);
		...
		gen(v);
	} else if (outd[v] < 2 and outd[v+N] < 2) go(v+1);
	else {
		gen(v);
		gen(v+N);
	}
