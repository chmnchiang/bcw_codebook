...
int N,D,sz[MAXN],mid[MAXN];
vector<int> E[MAXN];
//cc_hash_table<int, vector<int>> mp[MAXN];
...
void DFS(int u, int f, int d) {
	...
	for (auto v:E[u]) {
		if (v == f) continue;
		if (v == hao) continue;
		for (auto &it:mp[mid[v]]) {
			if (it.S.empty()) continue;

			if (it.F - d >= D) {
				//for (auto z:it.S) mp[id][it.F].PB(z);
			} else {
				int q = D - (it.F - d) + d;
				int x = it.S[0];
				auto ptr = mp[id].find(q);
				if (ptr == end(mp[id])) {
					//for (auto z:it.S) mp[id][it.F].PB(z);
				} else {
					for (auto z:it.S) djs.uni(x, z);
					for (auto z:ptr->S) djs.uni(x, z);
					assert(!ptr->S.empty());
					ptr->S.resize(1);
					it.S.resize(1);
					//mp[id][it.F].PB(x);
