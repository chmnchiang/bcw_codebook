...
const ll TWOINV = 500000004;

int N, M;
map<pii, vector<int>> edge;
vector<pii> man_edge;
vector<int> tdge[MAXN];
int root[MAXN];
bool vis[MAXN];
bool elg[MAXN];
ll vc[MAXN];
...
pll dfs(int v)
{
	if(!elg[v] or vis[v]) return {1, 0};
	pll ans = {vc[v], 1};
	vis[v] = true;
	for(auto c: tdge[v])
	{
		pll tmp = dfs(c);
		ans.F = (ans.F * tmp.F) % MOD;
		ans.S += tmp.S;
	}
