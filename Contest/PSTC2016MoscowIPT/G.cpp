...
double check(int n)
{
	double al = sqrt(n * P * (1-P)), be = n * P;
	vector<int> buck;
	vector<vector<double>> rbuck;
	buck.resize(n+1);
	rbuck.resize(n+1);
	for(int i=0; i<N; i++)
	{
		double x = arr[i] * al + be; 
		int z = round(x);
		if(z < 0 or z > n) return INF;
		buck[z]++;
		double d = x - z;
		rbuck[z].PB(d);
	}
	...
	for(int k=0; k<=n; k++)
	{
		double pk = comb[n][k] * pow(P, k) * pow(1-P, n-k);
		double ev = pk * N;
		double var = sqrt(N * pk * (1-pk));

		double dif = (buck[k] - ev) / var;
//		ans += dif * dif;
