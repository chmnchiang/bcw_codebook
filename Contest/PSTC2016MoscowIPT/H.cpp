...
struct SegmentTree {
	...
	void push(int id, int lc, int rc) {
		...
		tree[lc].val.F += tree[id].tag;

		tree[rc].tag += tree[id].tag;
		tree[rc].val.F += tree[id].tag;
	...
	pii qry_tree(int l, int r, int ql, int qr, int id) {
	...
	pii qry(int ql, int qr) {
		return qry_tree(0,n,ql,qr,1);
	}
...
//	cout << endl;
	...
	for (auto qry: ip) {
		...
		if (typ == 1) {
			...
			auto nextIt = avail.lower_bound(val);
			int nextVal = *nextIt;

			avail.erase(nextIt);
			ans += nextVal - val;

			auto rpos = upper_bound(Q, Q+M, pii(nextVal, INF)) - Q;
			auto prevUsedPos = usedPos.upper_bound(ps);

			int myval;
			if (prevUsedPos == usedPos.begin()) {
				myval = val;
			} else {
				--prevUsedPos;
				int t = *prevUsedPos;
				//cout << "Qry t = " << t << endl;
				myval = max(val, tree.qry(t) + Q[t].F + 1);
			}

			tree.add(ps+1, rpos-1, 1);
//			cout << "ADD = " << ps+1 << " ~ " << rpos-1 << endl;
//			cout << "Myval = " << myval << ' ' << "Val = " << val << endl;
			tree.upd(ps, myval - val);
//			cout << "Update " << ps << ' ' << myval - val << endl;
			usedPos.insert(ps);
		} else {
			int ps = pos[qry.S];
			int val = Q[ps].F;
...
//			cout << "[-]" << endl;
			//cout << "ps = " << ps << ' ' << qry.S << endl;

			int myval = tree.qry(ps) + val;
//			cout << "Myval = " << myval << endl;
			auto rpos = upper_bound(Q, Q+M, pii(nextVal, INF)) - Q;
			tree.upd(ps, INF);
			auto p = tree.qry(ps, rpos-1);
//			cout << p << endl;
			if (p.F == 0) {
				nextVal = Q[p.S].F;
				rpos = p.S;
