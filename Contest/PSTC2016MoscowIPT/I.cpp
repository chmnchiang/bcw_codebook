...
int main() {
	...
	while(cin>>U>>X0>>Y0>>N)
	{
		...
		for(int i=0; i<=N; i++)
		{
			...
			double cth = max(dot(q, p1) / (abs(q) * abs(p1)), dot(q, p2) / (abs(q) * abs(p2)));
			if(0 < s and s < 1) cth = 1;

			double val = abs(v[i]) / abs(q) * cth * cth;
			ans = max(ans, val);

//			cout<<"S = "<<s<<" Q = "<<q<<" CTH "<<cth<<endl;
//			cout<<p1<<" => "<<p2<<endl;
			p1 = p2;
		}
