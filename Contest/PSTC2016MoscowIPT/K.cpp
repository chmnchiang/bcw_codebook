...
struct SegmentTree {
	...
	void push(int id) {
		...
		pii tmp;
		tree[lc].tag = min(tree[lc].tag, tree[id].tag);
		tmp = {tree[lc].mn.F + tree[lc].tag, tree[lc].mn.S};
		tree[lc].val = min(tree[lc].val, tmp); 

		tree[rc].tag = min(tree[rc].tag, tree[id].tag);
		tmp = {tree[rc].mn.F + tree[rc].tag, tree[rc].mn.S};
		tree[rc].val = min(tree[rc].val, tmp); 
	}
	void init(int l, int r, int id, int arr[]) {
		if (l == r) {
			tree[id].mn = tree[id].val = {arr[l], l};
			tree[id].val.F += INF;
			tree[id].tag = INF;
			return;
		}
	...
	void del(int l, int r, int q, int id) {
		if (l == r) {
			tree[id].mn.F = tree[id].val.F = tree[id].tag = INF;
			return;
		}
		int m = (l+r)/2, lc = id*2, rc = id*2+1; 
		push(id);
		...
		tree[id].mn = min(tree[lc].mn, tree[rc].mn);
		tree[id].val = min(tree[lc].val, tree[rc].val);
//		cout<<"DEL "<<l<<" "<<r<<" "<<id<<" "<<tree[id].val<<endl;
	}
	void upd(int l, int r, int ql, int qr, int v, int id) {
		if (r < ql) return;
		if (qr < l) return;
		if (ql <= l and r <= qr) {
			tree[id].tag = min(tree[id].tag, v);
			pii tmp = {tree[id].mn.F + tree[id].tag, tree[id].mn.S};
			tree[id].val = min(tree[id].val, tmp);
			return;
		}
		int m = (l+r)/2, lc = id*2, rc = id*2+1; 
		push(id);
		upd(l,m,ql,qr,v,lc);
		upd(m+1,r,ql,qr,v,rc);
		tree[id].mn = min(tree[lc].mn, tree[rc].mn);
		tree[id].val = min(tree[lc].val, tree[rc].val);
//		cout<<"UPD "<<l<<" "<<r<<" "<<id<<" "<<tree[id].val<<endl;
	}
	/*
	pii qry(int l, int r, int ql, int qr, int id) {
		if (r < ql) return {INF, INF};
		if (qr < l) return {INF, INF};
		if (ql <= l and r <= qr) return tree[id].val;
		int m = (l+r)/2, lc = id*2, rc = id*2+1; 
		push(id);
		return min(qry(l,m,ql,qr,lc), qry(m+1,r,ql,qr,rc));
	}
	*/
} tree1, tree2;
...
int main() {
	...
	REP1(i,1,N) vec.PB(A[i]);

	tree1.upd(1,N,1,1,0,1);
	tree2.upd(1,N,1,1,0,1);
	long long ans = -A[1];
	REP(_,N) {
		pii r1 = tree1.tree[1].val;
		pii r2 = tree2.tree[1].val;
		pii res = min(r1, r2);

		int u = res.S;
//		cout<<"CHOOSE "<<u<<" WITH COST "<<res.F<<endl;
//		cout<<r1<<" "<<r2<<endl;
		...
		int x = lower_bound(ALL(vec), M-A[u]) - begin(vec) + 1;

		if (vec[0] < M-A[u]) {
//			cout<<"UPDATE 1: "<<x-1<<" "<<A[u]<<endl;
			tree1.upd(1,N,1,x-1,A[u],1);
		}
		if (vec.back() >= M-A[u]) {
//			cout<<"UPDATE 2: "<<x<<" "<<A[u]-M<<endl;
			tree2.upd(1,N,x,N,A[u]-M,1);
		}
