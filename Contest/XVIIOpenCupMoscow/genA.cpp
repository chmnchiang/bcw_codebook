...
int T,N;
int w[MAXN][MAXN],done[MAXN],dis[MAXN],prv[MAXN];
vector<int> E[MAXN];
vector<pii> edge, edge2;

void gen1() {
	REP1(i,1,N) REP1(j,1,i-1) w[i][j] = w[j][i] = rand();
	REP1(i,1,N) done[i] = 0, dis[i] = INF;
	dis[1] = 0;
	edge.clear();
	REP(_,N) {
		int u = -1;
		REP1(v,1,N) {
			if (done[v]) continue;
			if (u == -1 or dis[v] < dis[u]) u = v;
		}
		done[u] = 1;
		if (_) edge.PB({prv[u],u});
		REP1(v,1,N) {
			if (done[v]) continue;
			if (dis[v] > w[u][v]) {
				dis[v] = w[u][v];
...
int fr,bk,que[MAXN];
int sz[MAXN],ban[MAXN],vst[MAXN],fa[MAXN];
...
int centroid(vector<int> vec) {
	if (SZ(vec) == 1) return vec[0];
	int n = SZ(vec);
	for (auto i:vec) ban[i] = vst[i] = 0;
	...
	ban[cen] = 1;
	for (auto i:vec) vst[i] = 0;
	for (auto v:E[cen]) {
		if (ban[v]) continue;
		vector<int> vvv;
		BFS(v);
		REP(i,bk) vvv.PB(que[i]);
		todo.PB(vvv);
	}
	
	for (auto i:vec) ban[i] = vst[i] = 1;
