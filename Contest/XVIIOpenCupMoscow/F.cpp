...
struct SAM{
	...
	void extend(int w){
		hsh[mylen] = (mylen ? hsh[mylen-1] : 0) * 2 + w;
		...
		if (p == 0){
			...
			if (vec[vec[p].go[w]].val == vec[p].val+1){
				...
				if (other == q) {
					int l = vec[vec[q].par].val + 1, rr = vec[q].val;
					if (olen > rr or olen < l) {
						other = r;
					}
				}
			}
	...
	void walk(int c) {
		while (other != root and !vec[other].go[c]) {
			other = vec[other].par;
			olen = min(olen, vec[other].val);
		}

		if (vec[other].go[c]) {
			olen++;
			other = vec[other].go[c];
		}
	}
...
int main() {
	...
	for (int i=0; i<Q; i++) {
		int c = S[i];
		int w = c - '0';
		int a = (X^w) % 2, d = (X^w)/2 % 2;
		//cin >> a >> d;

		sam[a].extend(d);
		sam[!a].walk(d);
		X = max(max(X, sam[0].olen), sam[1].olen);
		ans += X;
		//cout << "X = " << X << endl;
		//cout << "-----" << endl;
		//sam[0].output();
