...
struct SegmentTree {
	...
	void init(int _n) {
		n = _n;
		REP(i,MAXN*4) tree[i] = -INF;
	}
	void update_tree(int l, int r, int q, int x, int id) {
		if (l == r) {
			tree[id] = x;
			return;
		}
		int m = (l+r)/2, lc = id*2, rc = id*2+1;
		if (q <= m) update_tree(l,m,q,x,lc);
		else update_tree(m+1,r,q,x,rc);
		tree[id] = max(tree[lc], tree[rc]);
	}
	int query_tree(int l, int r, int ql, int qr, int id) {
		if (qr < l) return -INF;
		if (r < ql) return -INF;
		if (ql <= l and r <= qr) return tree[id];
		int m = (l+r)/2, lc = id*2, rc = id*2+1;
		return max(query_tree(l, m, ql, qr, lc), query_tree(m+1, r, ql, qr, rc));
	}

	void upd(int i, int x) {
		update_tree(0,n,i,x,1);
	}
	int qry(int l, int r) {
		return query_tree(0,n,l,r,1);
	}
} tree ;
...
int main() {
	...
	for (int i=1; i<=N; i++) {
		if (i >= L) {
			int t = i - L;
			int id = lower_bound(ALL(pr), pii(S[t], t)) - pr.begin();
			//cout << "PUSH " << id << endl;
			tree.upd(id, dp[t]);
		}
		if (i >= R+1) {
		...
		int &ans = dp[i];
		int ql = lower_bound(ALL(pr), pii(S[i], -INF)) - pr.begin();
		int qr = lower_bound(ALL(pr), pii(S[i], INF)) - pr.begin();

		int a1 = tree.qry(0, ql-1);
		int a2 = tree.qry(ql, qr-1);
		int a3 = tree.qry(qr, N);

		ans = max(a1+1, max(a2, a3-1));
	}
