...
const int MAXN = 105;
const pll dir[4] = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};
...
void pre()
{
	...
	for(int i=0; i<N; i++)
	{
		_pt[i].F = rand() % 2000000 - 1000000;
		_pt[i].S = rand() % 2000000 - 1000000;
	}
#endif
}
...
pll trim(pll x)
{
	x.F = max(x.F, -1000000LL);
	x.S = max(x.S, -1000000LL);
	x.F = min(x.F, 1000000LL);
	x.S = min(x.S, 1000000LL);
	return x;
}

int query(pll p, ll r)
{
	cnt++;
	ll r2 = r * r;
	assert(abs(p.F) <= 1000000 and abs(p.S) <= 1000000);
	int x = 0;
#ifdef HAO123
	for(int i=0; i<N; i++)
		if(abs2(p - _pt[i]) <= r2)
