...
lld eval(lld th)
{
	lld uC = u * cosl(th + PI/2 - phi);
	lld x = -uC + sqrtl(uC * uC + vvmuu);
	return R / x;
}
...
void calc_T()
{
	...
	{
		sum[i+2] = sum[i] + (val[i] + 4 * val[i+1] + val[i+2]) * rat;
		lld z1 = sum[i]   + (val[i]   + val[i+1]) / 2 * (2 * PI / STEP);
		lld z2 = sum[i+2] - (val[i+1] + val[i+2]) / 2 * (2 * PI / STEP);
		sum[i+1] = (z1 + z2) / 2;
	}
	T = sum[STEP];

	cout<<setprecision(15)<<"T = "<<(ld)T<<setprecision(9)<<endl;
}

void query(ld tt)
{
	int cycle = floorl(tt / T);
	ld t = tt - cycle * T;
	//cout<<tt<<" : CYCLE = "<<cycle<<" REM = "<<t<<endl;
	...
	ld rem = tt - sum[lb];
	ld v1 = val[lb], v2 = val[lb+1];
	ld d = (v2 - v1) / (2 * PI / STEP);
	ld z = 0;
	if(abs(d) < 1E-6) z = rem / v1;
	else z = (sqrtl(v1 * v1 + 2 * d * rem) - v1) / d;

	ld th = lb * 2 * PI / STEP + z;
	ld x = px + tt * ux + R * cosl(th + th0);
	ld y = py + tt * uy + R * sinl(th + th0);
...
int main() {
	...
	ld dx = qx - px, dy = qy - py;
	R = hypotl(dx, dy);
	th0 = atan2l(dy, dx);
	u = hypotl(ux, uy);
	if(u < 1E-10) phi = 0;
	else phi = atan2l(uy, ux) - th0;
	vvmuu = v * v - u * u;

	//cout<<"R "<<R<<" th0 "<<th0<<" phi "<<phi<<" u "<<u<<endl;
