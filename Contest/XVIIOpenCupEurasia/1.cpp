...
int N,K;
ll A[MAXN],dp[4][MAXN][12][12];
char prv[4][MAXN][12][12];
...
int main() {
	...
	REP1(r,1,3) {
		REP1(i,0,N) {
			REP(j,11) REP(k,11) dp[r][i][j][k] = -INF;
			dp[1][i][0][0] = 0LL;
		}
	}
	REP1(r,1,3) {
		REP1(i,0,N) {
			REP1(j,0,K) {
				REP1(k,0,K) {
					if (dp[r][i][j][k] == -INF) continue;
					if ((r == 1 or r == 3) and dp[r][i+1][j][k+1] < dp[r][i][j][k] + A[i+1]) {
						// move in A[i+1]
						dp[r][i+1][j][k+1] = dp[r][i][j][k] + A[i+1];
						prv[r][i+1][j][k+1] = 1;
					}
					if ((r == 1 or r == 3) and dp[r][i+1][j][k] < dp[r][i][j][k]) {
						// ignore A[i+1]
						dp[r][i+1][j][k] = dp[r][i][j][k];
						prv[r][i+1][j][k] = 2;
					}
					if (r == 2 and dp[r][i+1][j][k] < dp[r][i][j][k] + A[i+1]) {
						// choose element in segment
						dp[r][i+1][j][k] = dp[r][i][j][k] + A[i+1];
						prv[r][i+1][j][k] = 3;
					}
					if (r == 2 and dp[r][i+1][j+1][k] < dp[r][i][j][k]) {
						// move out A[i+1]
						dp[r][i+1][j+1][k] = dp[r][i][j][k];
						prv[r][i+1][j+1][k] = 4;
					}
					if (r <= 2 and dp[r+1][i][j][k] < dp[r][i][j][k] ) {
						// yang sheng !111111111!
						dp[r+1][i][j][k] = dp[r][i][j][k];
						prv[r+1][i][j][k] = 5;
					}
				}
			}
	...
	int bj = 0;
	REP1(j,0,K) {
		if (dp[3][N][j][j] > dp[3][N][bj][bj])
			bj = j;
//		cout<<j<<": "<<dp[3][N][j][j]<<endl;
	}
	if (dp[3][N][bj][bj] == 0) {
		int bi = 1;
		REP1(i,1,N) if (A[i] > A[bi]) bi = i;
		cout<<A[bi]<<" "<<0<<endl;
	} else {
		...
		int k = bj;
		while (i >= 0) {
//			cout<<r<<" "<<i<<" "<<j<<" "<<k<<" : "<<dp[r][i][j][k]<<" "<<prv[r][i][j][k]<<endl;
			assert(r >= 1);
			assert(j >= 0);
			assert(k >= 0);
			if (prv[r][i][j][k] == 5) {
...
//		cout<<sl<<" "<<sr<<endl;
		...
		cout<<dp[3][N][bj][bj]<<" "<<bj<<endl;
		REP(x,bj) cout<<pp[x]<<" "<<qq[x]<<endl;
		cout<<sl<<" "<<sr<<endl;
	}
