...
int main() {
	...
	for (int i=0; i<X; i++) {
		int id = i+N+1;
		flow.add_edge(ss, id, 3);
		int u = cont[i].F, v = cont[i].S;
		flow.add_edge(id, u, 3);
		flow.add_edge(id, v, 3);
	}

	int s = 0;
	for (int i=1; i<=N; i++) {
		int p; cin >> p;
		flow.add_edge(i, tt, p - P[i]);
		s += p - P[i];
	}
	...
	for (int i=0; i<X; i++) {
		...
		for (auto &e: el) {
			...
			int u = cont[i].F == v ? cont[i].S : cont[i].F;
			u--; v--;

			int f = e.of - e.f;
			if (f == 3) ip[v][u] = 'W';
			else if (f == 2) ip[v][u] = 'w';
			else if (f == 1) ip[v][u] = 'l';
			else ip[v][u] = 'L';
		}
	}

	for (int i=0; i<N; i++) cout << ip[i] << endl;
