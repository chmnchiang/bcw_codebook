...
int main() {
	...
	else
	{
		...
		for(int i=5; i<=N; i++)
		{
			if(i%4==1) vans.push_front(vans[0] + pii(3, -4));
			if(i%4==2) vans.push_front(vans[0] + pii(3, 4));
			if(i%4==3) vans.PB(vans.back() + pii(3, 4));
			if(i%4==0) vans.PB(vans.back() + pii(3, -4));
		}

		for(auto &p: vans)
		{
			p = pii(5*p.F+12*p.S, 12*p.F-5*p.S);
		}
	}

	for(int i=0; i<N; i++)
	{
		pii a = vans[i], b = vans[(i+1)%N];
