...
string genBig() {
	while (true) {
		//cout << opc << ' ' << digc << ' ' << cntendl;
		if (!cnt["("]) roll(2);
		else if (cnt["("] < cnt[")"]) roll(2);
		else if (cnt["("] > cnt[")"]) roll(4);
		else if (opc > digc - 3) roll(4);
		else break;
	}
	string ans = "";
...
int main() {
	...
	sort(ALL(ans));
	sort(ALL(SSS));
	//cout << ans << endl << SSS << endl;
	//cout << "TS = "<<ts<<endl;
	//int len = SZ(ans);
	//cout << len << endl;
	//cout<<ans<<endl;
	//cout<<SSS<<endl;
	assert(SZ(ans) == SZ(SSS));
	assert(SSS == ans);
	assert(SZ(SSS) < 1000);
