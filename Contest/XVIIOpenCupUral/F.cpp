...
Point operator-(Point a, Point b){return {a.x-b.x, a.y-b.y, a.z-b.z};};
Point operator*(ld a, Point b){return {a*b.x, a*b.y, a*b.z};};
ld dot(Point a, Point b){return a.x*b.x+a.y*b.y+a.z*b.z;}
ld abs(Point a){return sqrtl(dot(a, a));}
Point unit(Point a){return (1.l / abs(a)) * a;}
...
ld elapsed;
ld _start_time;
ld tar_phi, tar_lambda;
Point _target;
Point _p, _n;
int qcnt;

pdd get_coordinate(Point p=_p)
{
	p = unit(p);
	ld phi, lam;
	if(hypot(p.x, p.y) < EPS)
		lam = 0;
	else
		lam = atan2l(p.y, p.x);
	phi = asin(p.z);
...
pdd latlon(Point p=_p)
{
	pdd res = get_coordinate(p);
	return {res.F * 180 / PI, res.S * 180 / PI};
}

int _timezone()
{
	ld lam = get_coordinate(_p).S;
	if(lam < -PI + PI/24 or lam > PI - PI/24) return 12;
	int tz = roundl(lam * 24 / (2 * PI));
	tz = ((tz + 11) % 24) - 11;
//	cout<<latlon()<<" TZ "<<tz<<endl;
	return tz;
}

ld _localtime()
{
	return elapsed + _timezone();
}
...
Point rand_point()
{
	return unit(Point({2*frand()-1, 2*frand()-1, 2*frand()-1}));
}

int query(ld rot, ld th)
{
	qcnt++;
	ld dis = th * 20000 / PI;
	ld dt = dis / 180 + abs(rot) / (480 * PI);
	ld rotdeg = rot * 180 / PI;
	assert(qcnt <= 25000);
	assert(0 <= dis and dis < 40000);
	assert(-360 < rotdeg and rotdeg < 360);
	elapsed += dt;
	ld tm;
#ifdef HAO123
	Point _k = unit(cross(_p, _n));
	_n = unit(cosl(rot) * _n + sinl(rot) * _k);
	Point _f = unit(cross(_n, _p));
	_p = unit(cosl(th) * _p + sinl(th) * _f);
	tm = _localtime() - _start_time;
#else
	string s1, s2;
	cout<<rotdeg<<" "<<dis<<endl;
	cin>>s1>>s2;
	if(s1 == "---") exit(0);
	int week = 0;
	if(s1 == "mon") week = 0;
	...
	if(s1 == "sat") week = 5;
	if(s1 == "sun") week = 6;
	int hh = (s2[0]-'0') * 10 + (s2[1]-'0');
	int mm = (s2[3]-'0') * 10 + (s2[4]-'0');
	tm = week * 24 + hh + (mm / 60.l);
#endif
	ld dif = tm - elapsed;
	dif = fmod(dif, 168);
	if(dif < -84) dif += 168;
	...
	int tzdiff = round(dif);
	return tzdiff;
}

int realfly(ld rot, ld th)
{
	int res;
	if(th > 0)
		res = query(rot, th);
...
//	cout<<curtz<<" "<<latlon()<<endl;

	ld lb = 0, rb = PI;
	for(int i=0; i<100; i++)
	{
		ld mb = (lb + rb) / 2;
		int newtz = tryfly(0, mb);
		if(newtz != curtz)
			rb = mb;
		else
			lb = mb;
...
//	cout<<lb<<" "<<curtz<<" "<<latlon()<<endl;

	int tzplus = tryfly(0, 0.0001);
	int tzminus = tryfly(0, -0.0001);
	int tzdif = (((tzplus - tzminus) % 24) + 24) % 24;
	if(tzdif != 1)
	{
		query(PI, 0);
		swap(tzplus, tzminus);
...
//	cout<<curtz<<" "<<latlon()<<endl;
	...
	for(int i=0; i<100; i++)
	{
		ld mb = (lb + rb) / 2;
		int res = tryfly(0, mb);
		if(res == tzplus or res == tzminus) lb = mb;
		else rb = mb;
	}
...
//	cout<<curtz<<" "<<latlon()<<endl;
	...
	curtz = query(-PI/2, PI/24);

//	cout<<curtz<<" "<<latlon()<<endl;
	while(1)
	{
		int nxttz = query(0, PI/12);
		int dif = abs(nxttz - curtz);
//		cout<<curtz<<" => "<<nxttz<<" "<<latlon()<<endl;
		curtz = nxttz;
		if(dif != 1)
			break;
	}
...
//	cout<<curtz<<" "<<latlon()<<endl;
	curtz = query(0, PI*11/12);
//	cout<<curtz<<" "<<latlon()<<endl;

	realfly(0, tar_lambda);
	realfly(PI/2, tar_phi);

//	cout<<latlon()<<endl;
//	cout<<latlon(_target)<<endl;
	
//	ld pdif = abs(_target - _p) * 40000 / (2 * PI);
//	cout<<pdif<<" km"<<endl;
...
#ifdef HAO123
	_target = unit(rand_point());
	tie(tar_phi, tar_lambda) = get_coordinate(_target);
	_p = rand_point();
	_n = rand_point();
	_n = unit(_n - dot(_n, _p) * _p);
	_start_time = _localtime();

#else
	cin>>tar_phi>>tar_lambda;
	tar_phi *= PI / 180;
	tar_lambda *= PI / 180;
	string s;
	cin>>s;
