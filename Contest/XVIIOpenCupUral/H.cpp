...
int main() {
	...
	sort(ALL(pset));
	pset.resize(unique(ALL(pset))-begin(pset));

	bs.resize(SZ(pset));
	for (auto &i: bs) i.reset();

	REP(i,M) {
		for (auto v:cafe[i].vec) {
			int id = lower_bound(ALL(pset),v) - begin(pset);
			bs[id][i] = 1;
		}
	}
	...
	REP(i,N) {
		...
		for (auto v: group[i]) {
			int id = lower_bound(ALL(pset), v) - begin(pset);
			if (id == SZ(pset) or pset[id] != v) continue;
			REP(j,M) {
				if (bs[id][j]) cnt[j]++;
			}
		}
		...
		int best = 0;
		REP(j,M) if (cnt[j] > cnt[best]) best = j;
		cout<<best+1<<endl;
	}
