...
ll encode(const int vec[]) {
	ll res = 0;
	REP1(i,0,N) res = (res << 2) + vec[i];
	return res;
}

void decode(ll s, int vec[]) {
	for (int i=N; i>=0; i--) {
		vec[i] = s & 3;
		s >>= 2;
	}
...
int main() {
	...
	REP1(i,1,N) {
		if (ip[0][i] != '#' and ip[N+1][i] != '#' and ip[0][i] != ip[N+1][i]) {
			mul2cnt++;
		}
		if (ip[i][0] != '#' and ip[i][N+1] != '#' and ip[i][0] != ip[i][N+1]) {
			mul2cnt++;
		}
		if (ip[0][i] != '#') cntC[i]++;
		if (ip[N+1][i] != '#') cntC[i]++;
	...
	REP1(i,1,N) {
		REP1(j,1,N) {
			...
			sort(ALL(vs[he]));
			vs[he].resize(unique(ALL(vs[he]))-begin(vs[he]));
