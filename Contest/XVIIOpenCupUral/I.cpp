...
void place(pii p, pii o, int r) {
	p = rotate(p, r) + o;
	mp[49-p.S][p.F] = '#';
}

void checker() {
	const int ddir[4][2] = {{-1,0},{0,1},{1,0},{0,-1}};
	int d = 1;
	int x = -1, y = -1;
	REP(i,50) REP(j,50) {
		if (mp[i][j] == '@') {
	...
	int nx = x + ddir[d][0];
	int ny = y + ddir[d][1];
	while (inr(pii(nx,ny)) and mp[nx][ny] != '#') {
		x = nx;
		y = ny;
		nx = x + ddir[d][0];
		ny = y + ddir[d][1];
	}
	assert(inr(pii(nx,ny)));
	...
	REP(i,SZ(S)) {
		...
		if (i == SZ(S)-1) assert(!inr(pii(nx,ny)));
		else assert(inr(pii(nx,ny)));
	}
}
...
int main() {
	...
	for (int i=0; i<50; i++) {
		for (int j=0; j<50; j++) mp[i][j] = '.';
	}

	while (pos != N-1) {
		int left = N-pos;
		if (left >= 20) {
			string s = S.substr(pos, 6);
			if (s == "LLLLLL") {
				place({w-5, 2}, o, rot);
				place({w-4, 1}, o, rot);
				place({w-3, 3}, o, rot);
		...
		string s = S.substr(pos, 2);
		if (s == "LR") {
			place({1, 0}, o, rot);
			o = o + rotate({w-1, 1}, rot);
			tie(w, h) = pii(h-1, w);
			rot = (rot + 1) % 4;
		} else if (s == "RL") {
			place({1, h-1}, o, rot);
			o = o + rotate({0, h-2}, rot);
			tie(w, h) = pii(h-1, w);
			rot = (rot + 3) % 4;
		} else if (s == "LL") {
			place({w-1, 0}, o, rot);
			o = o + rotate({w-2, 1}, rot);
			tie(w, h) = pii(h-1, w-1);
			rot = (rot + 1) % 4;
		} else if (s == "RR") {
			place({w-1, h-1}, o, rot);
	...
	if (S[N-1] == 'L') place({w/2, 0}, o, rot);
	else place({w/2, h-1}, o, rot);
