...
inline bool inter(pii p, int x) {
	return p.F <= x and x <= p.S;
}

bool inter(pii p1, pii p2) {
	return inter(p1, p2.F) or inter(p1, p2.S) or inter(p2, p1.F) or inter(p2, p1.S);
}
...
int main() {
	...
	for (int i=0; i<N; i++) {
		flow.add_edge(S, i, F[i]);
	}

	for (int i=0; i<M; i++) {
		flow.add_edge(i+N, U, G[i]);
	}
