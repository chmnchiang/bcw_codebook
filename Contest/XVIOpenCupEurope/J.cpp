...
struct DisjointSet {
	...
	int ffa(int u) {
		return val[u] == u ? u : (val[u] = ffa(val[u]));
	}
...
void scc() {
	...
	for (int i=1; i<=V; i++) {
		for (auto v: el[i]) {
			if (i > v) continue;
			if (ban == pii{v, i} or ban == pii{i, v}) continue;
			graph.add_edge(i, v);
		}
	}
...
#define rand rraanndd
LL rand() {
#undef rand
	return (LL) rand() * INT_MAX + rand();
}
#define rand rraanndd
...
int main() {
	...
	for (int i=1; i<=V; i++) {
		for (auto u: el[i]) {
			...
			for (int j=1; j<=V; j++) zz[j] = rand();
			for (int j=1; j<=V; j++) hsh[j][2] ^= zz[graph.djs.ffa(j)];
	...
	for (int i=1; i<=V; i++) {
		for (int j=i+1; j<=V; j++) {
			for (int x=0; x<=2; x++) ans += hsh[i][x] == hsh[j][x];
		}
	}
	cout << ans << endl;
