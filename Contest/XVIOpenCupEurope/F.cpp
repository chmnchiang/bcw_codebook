...
ll calc()
{
	...
	ll v = 0;
	for(int i=1; i<=N-1; i++)
		v += (l[i] * bpow[N-1-i] * comb(2*N-3-i, N-2)) % MOD;
	v %= MOD;
	v = (v * apow[N-1]) % MOD;
	ans += v;
	...
	v = 0;
	for(int i=1; i<=N-1; i++)
		v += (t[i] * apow[N-1-i] * comb(2*N-3-i, N-2)) % MOD;
	v %= MOD;
	v = (v * bpow[N-1]) % MOD;
	ans += v;
	...
	for(int i=0; i<=N-2; i++)
	{
		ll z = (apow[i] * ifact[i]) % MOD;
		if(z > MOD/2) z -= MOD;
		arr[i] = z;
	}
	for(int i=0; i<=N-2; i++)
	{
		ll z = (bpow[i] * ifact[i]) % MOD;
		if(z > MOD/2) z -= MOD;
		brr[i] = z;
	}
