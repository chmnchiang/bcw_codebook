...
struct ZZZ {
	...
	void del(int i) {
		...
		mp.erase(i);
		A34 tmp;
		tmp[0] = dif; tmp[1] = i; tmp[2] = j;
		assert(s.find(tmp) != end(s));
		s.erase(s.find(tmp));
	}
	void add(int i, int j, int dif) {
		dif -= doff;
		A34 tmp;
		tmp[0] = dif; tmp[1] = i; tmp[2] = j;
		s.insert(tmp);
	...
	int get_min_d() {
		...
		return (*begin(s))[0] + doff;
	}
	vector<pii> pop() {
		vector<pii> res;
		for (auto it:s) {
			if (it[0] + doff > 0) break;
			res.PB({it[1], it[2]});
		}
		return res;
	}
...
int N,M,h[MAXN],prv[MAXN],nxt[MAXN],mask[MAXN],bye[MAXN];
int gsign[MAXN];
int ans[MAXN];
string str;
...
vector<int> getall() {
	vector<int> res;
	for (int i=nxt[0]; i != N+1; i=nxt[i]) res.PB(i);
	return res;
}
int isvvv(int i, int j, int k) {
	return (gsign[i] * gsign[j] < 0);
}
int suan(int j) {
	int i = prv[j];
	int k = nxt[j];
	int l = nxt[k];
	int res = 0;
	if (isvvv(i,j,k)) res |= 1;
	if (isvvv(j,k,l)) res |= 2;

	return res;
}
void go(int i) {
	int gap = hao123[mask[i]].getdif(i);
	hao123[mask[i]].del(i);
	mask[i] = suan(i);
	hao123[mask[i]].add(i,nxt[i],gap);
}
int main() {
	IOS;
	cin>>N>>M;
	...
	}
	{
		...
		for (auto i:vec) {
			int j = nxt[i];
			int gap = abs(h[i] - h[j]);
			if (h[i] < h[j]) gsign[i] = 1;
			else gsign[i] = -1;
			hao123[mask[i]].add(i,j,gap);
		}
	}
	int hh = h[1];
	for (auto c:str) {
//		for (int i=1; i<=N; i = nxt[i]) cout<<pii(i,gsign[i])<<" ";
//		cout<<endl;
		if (c == 'A') {
			hao123[1].move_on();
		} else {
		...
		//cout<<"MERGE "<<vec<<endl;
		{
			...
			for (int i=sz-1; i>=0; i--) {
				...
				int m = mask[x];
				hao123[m].del(x);
				int gap = hao123[mask[y]].getdif(y);
				hao123[mask[y]].del(y);
				mask[x] = mask[y];
				gsign[x] = gsign[y];
				hao123[mask[x]].add(x,nxt[y],gap);
				merge(x,y);
			}
		}
		for (auto it:vec) {
	...
	//cout<<"FINAL ";
	//for (int i=1; i<=N; i = nxt[i]) cout<<i<<" ";
	//cout<<endl;
	...
	for (int i=1; i<=N; i=nxt[i]) {
		...
		int sign = gsign[i];
		int gap = hao123[mask[i]].getdif(i);
		ans[j] = ans[i] + gap * sign;
	}

	for (int i=2; i<=N; i++) {
		if (bye[i]) ans[i] = ans[i-1];
	}
