...
pdd operator-(pdd a, pdd b){return {a.F-b.F, a.S-b.S};}
pdd operator*(double b, pdd a){return {a.F*b, a.S*b};}
...
bool inside(pdd p, pdd q, pdd x, double r)
{
	pdd d = q - p;
	if(abs(x-p) <= r or abs(x-q) <= r) return true;
	if(dot(x-p, d) >= 0 and dot(x-q, d) <= 0 and abs(cross(x-p, d)) <= abs(d) * r) return true;
	return false;
}

int calc(pii p, pii q)
{
	int dx = abs(p.F - q.F), dy = abs(p.S - q.S);
	int ans = 0;
	if(dx >= dy)
	{
		if(p.F > q.F) swap(p, q);
		for(int x=p.F; x<=q.F; x++)
		{
			int y = p.S + ((x - p.F) * (q.S - p.S)) / (q.F - p.F);
			for(int i=y-2; i<=y+2; i++)
			{
				if(i < 0 or i >= MAXR) continue;
				if(pr[x][i] < 0) continue;
				if(inside(p, q, {x, i}, pr[x][i])) ans++;
			}
		}
	}
	else
	{
		if(p.S > q.S) swap(p, q);
		for(int y=p.S; y<=q.S; y++)
		{
			int x = p.F + ((y - p.S) * (q.F - p.F)) / (q.S - p.S);
			for(int i=x-2; i<=x+2; i++)
			{
				if(i < 0 or i >= MAXR) continue;
				if(pr[i][y] < 0) continue;
				if(inside(p, q, {i, y}, pr[i][y])) ans++;
			}
		}
	}
