...
double calc_area(Line l)
{
	vector<Line> vec;
	for(int i=0; i<4; i++)
		vec.PB({pt[i], pt[(i+1)%4]});
	vec.PB(l);
	...
	{
		bool trash;
		pdd x = interPnt(vec[i], vec[(i+1)%S], trash);
		vpt.PB(x);
	}
...
double calc()
{
	...
	ans += 4 * 5 * calc_area({{-0.5, 0.}, {-0.5, 1.}});
	ans += 3 * 5 * calc_area({{0.5, 0.}, {0.5, -1.}});
	ans += 1 * 5 * calc_area({{0., -0.5}, {-1., -0.5}});
	ans += 6 * 5 * calc_area({{0., 0.5}, {1., 0.5}});
