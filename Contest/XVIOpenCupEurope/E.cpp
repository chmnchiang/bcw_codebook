...
ostream& operator << (ostream &o, DT &d) {
	cout << d.v << ' ' << d.e << endl;
	for (int i=0; i<4; i++) cout << d.deg[i] << ' ';
	cout << endl;
	return o;
}
int degree[MX];
...
static void dinit() {
	...
	for (int i=0; i<MX; i++) dt[i].v = 1, dt[i].deg[0] = 1;
}
...
pii answer(int u) {
	auto &d = dt[u];
	if (d.e == 0) return {0, 0};
	if (d.v == d.deg[2]) return {1, 1};

	return {d.v - d.deg[2], d.e - d.deg[2]};
}
...
void proc(pii edge) {
	...
	du++; dv++;
	dt[fu].deg[min(du-1, 3)] --;
	dt[fu].deg[min(dv-1, 3)] --;
	dt[fu].deg[min(du, 3)] ++;
	dt[fu].deg[min(dv, 3)] ++;

	add(answer(fu), 1);
	//cout << dt[fu] << endl;
	//cout << u << ' ' << v << ": " << curV << ' ' << curE << endl;
}
