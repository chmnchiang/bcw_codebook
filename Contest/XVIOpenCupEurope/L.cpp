...
const int MAXN = 422;
const pii BAD = {34021501, 0x34021501};

int N, M, Q, S;
bool arr[MAXN][MAXN];
int root[MAXN*MAXN];
vector<int> vlx;
vector<pip> edge[MAXN];
bool vis[MAXN][MAXN][MAXN];
pii good_offset[MAXN];
bool all_ok, no_ok;
pii bas;
...
void calc()
{
	...
	for(int i=0; i<N; i++)
		for(int j=0; j<M-1; j++)
			if(arr[i][j] and arr[i][j+1])
				djUnion(i*M+j, i*M+j+1);
	for(int i=0; i<N-1; i++)
		for(int j=0; j<M; j++)
			if(arr[i][j] and arr[i+1][j])
				djUnion(i*M+j, (i+1)*M+j);
	...
	for(int i=0; i<N; i++)
	{
		if(arr[i][0]) vlx.PB(root[i*M]);
		if(arr[i][M-1]) vlx.PB(root[i*M+M-1]);
	}
	for(int i=0; i<M; i++)
	{
		if(arr[0][i]) vlx.PB(root[i]);
		if(arr[N-1][i]) vlx.PB(root[(N-1)*M+i]);
	}
	sort(ALL(vlx));
	S = unique(ALL(vlx)) - vlx.begin();
	vlx.resize(S);
	...
	for(int i=0; i<N; i++)
	{
		for(int j=0; j<M; j++)
		{
			int idx = lower_bound(ALL(vlx), root[i*M+j]) - vlx.begin();
			if(idx == S or vlx[idx] != root[i*M+j])
				root[i*M+j] = -1;
			else
				root[i*M+j] = idx;
		}
	...
	queue<pip> q;
	q.push({root[0], {MAXN/2, MAXN/2}});
	vis[root[0]][MAXN/2][MAXN/2] = true;
	while(!q.empty())
	{
		pip p = q.front();
		q.pop();
		...
		{
			int u = e.F;
			int nx = x + e.S.F, ny = y + e.S.S;
			if(nx < 0 or nx >= MAXN or ny < 0 or ny >= MAXN) continue;
			if(vis[u][nx][ny]) continue;
			vis[u][nx][ny] = true;
			q.push({u, {nx, ny}});
		}
	}
	...
	for(int i=0; i<S; i++)
	{
		...
		for(int j=0; j<MAXN; j++)
			for(int k=0; k<MAXN; k++)
				if(vis[i][j][k])
					good_offset[i] = {j-MAXN/2, k-MAXN/2};
	}

	if(vis[root[0]][MAXN/2][MAXN/2+1] and vis[root[0]][MAXN/2+1][MAXN/2])
	{
		all_ok = true;
		return;
	}
	...
	for(int i=0; i<MAXN; i++)
	{
		for(int j=0; j<MAXN; j++)
		{
			if(!vis[root[0]][i][j]) continue;
			int x = i - MAXN/2, y = j - MAXN/2;
...
bool query(int x, int y)
{
	...
	if(xm < 0) xm += N;
	if(ym < 0) ym += M;
	int xq = (x - xm) / N, yq = (y - ym) / M;

	int v = root[xm*M+ym];
	if(v == -1) return false;
	if(good_offset[v] == BAD) return false;

	if(all_ok) return true;
	if(no_ok) return pii{xq, yq} == good_offset[v];

	int dx = xq - good_offset[v].F, dy = yq - good_offset[v].S;
	int bx = bas.F, by = bas.S;
	int k = 0;
	if(abs(bx) > 0) k = dx / bx;
	else k = dy / by;
	return dx == k * bx and dy == k * by;
}
