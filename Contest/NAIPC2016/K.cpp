...
int N,ban[MAXN];
int fr,bk,que[MAXN],vst[MAXN],sz[MAXN],fa[MAXN];

ll ans[MAXN];
ll P[MAXN],dis[MAXN];
vector<pll> E[MAXN];
pll car[MAXN],deq[MAXN],qry[MAXN];
...
void go(const vector<int> &vvv) {
	if (vvv.empty()) return;
	//cout<<vvv<<endl;
	for (auto v:vvv) vst[v] = 0;
	BFS(vvv[0]);
	...
	for (auto v:vvv) {
		qry[nc] = pll(P[v],v);
		car[nc++] = pll(P[v], dis[v]);
	}
	sort(car,car+nc);
	reverse(car,car+nc);
	fr = bk = 0;
	REP(i,nc) {
		while (bk > fr and deq[bk-1].S > car[i].S) {
			bk--;
		}
		while (bk - fr >= 2) {
			auto a = deq[bk-2];
			auto b = deq[bk-1];
//			if ((car[i].S - a.S) * (a.F - b.F) <= (b.S - a.S) * (a.F - car[i].F)) bk--;
			if ((car[i].S - b.S) * (a.F - b.F) <= (b.S - a.S) * (b.F - car[i].F)) bk--;
			else break;
		}
		deq[bk++] = car[i];
	}
	...
	REP(i,nc) {
		int id = qry[i].S;
		ll t = qry[i].F;
		while (bk - fr >= 2 and at(deq[fr], t) >= at(deq[fr+1], t)) fr++;
//		for (int j=fr; j<bk; j++)
//			ans[id] = min(ans[id], at(deq[j], t) + dis[id]);
		ans[id] = min(ans[id], at(deq[fr], t) + dis[id]);
	}
	...
	for (auto it:E[cen]) {
		...
		BFS(v);
		vec.resize(bk);
		for (int i=0; i<bk; i++) vec[i] = que[i];
		go(vec);
	}
