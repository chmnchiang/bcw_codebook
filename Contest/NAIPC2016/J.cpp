...
int R,C,N;
BS bmp,cmp;
vector<vector<int>> mp, arr, brr;

ostream& operator << (ostream &s, const Line &x) {
	return s<<"{"<<x.p<<" -> "<<x.q<<"}";
}

inline int getID(int x, int y) { return x * C + y; }
inline ll abs(Line x) { return abs(x.p.F-x.q.F) + abs(x.p.S-x.q.S) + 1; }
BS gen(const vector<Line> vec) {
	BS res;
	arr.resize(R);
	REP(i,R) {
		arr[i].resize(C+1);
...
/*
	cout<<vec<<endl;
	REP(i,R) {
		REP(j,C) cout<<(int)res[getID(i,j)];
		cout<<endl;
	}
	*/
...
ll calc(ll mid) {
	...
	REP(i,N) {
		...
		if (tot + len <= mid) {
			...
			int dx = ip[i].q.F - ip[i].p.F;
			int dy = ip[i].q.S - ip[i].p.S;
			if (dx) dx /= abs(dx);
			if (dy) dy /= abs(dy);

			pii a,b;
			a = ip[i].p;
			b = pii(ip[i].p.F + dx * (mid-tot-1), ip[i].p.S + dy * (mid-tot-1));
			vec1.PB(Line{a,b});
	...
	REP(i,R) {
		REP(j,C) {
			cout<<(b1[getID(i,j)]&!b2[getID(i,j)]);
		}
		cout<<endl;
	}
	cout<<"CNT ="<<res<<endl;
...
int main() {
	...
	while (cin>>R>>C>>N) {
		...
		REP(i,N) {
			...
			ip[i].q = pii(x,y);
			assert(0 <= x and x < R and 0 <= y and y < C);
			tot += abs(ip[i]);
		}
