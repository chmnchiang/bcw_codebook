...
ll cst[1<<MAXN], dp[1<<MAXN][MAXN];

void dfs(int i, int j, int msk, int pos)
{
	if(pos == N)
	{
		dp[i][j] = min(dp[i][j], dp[i-msk][j-1] + cst[msk]);
	}
	else
	{
		dfs(i, j, msk, pos+1);
		if(i&(1<<pos))
			dfs(i, j, msk+(1<<pos), pos+1);
	}
}
...
//				cout<<bitset<5>(i)<<" "<<bitset<5>(x)<<endl;
				int y = i - x;
				dp[i][j] = min(dp[i][j], dp[y][j-1] + cst[x]);
			}
		}
	}
