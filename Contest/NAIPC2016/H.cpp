...
int N, K;
vector<ll> vecs[MAXN];
ll arr[MAXK], sum[MAXK];

ll dp[MAXK], tmp[MAXK];
ll dpr[MAXK], tmpr[MAXK];

void dfs(int lb, int rb, int dl, int dr)
{
	if(lb > rb) return;
	int mb = (lb + rb) / 2;

	ll val = -1, idx = -1;
	for(int i=dl; i<=dr and i<=mb; i++)
	{
		ll v = tmpr[i] + sum[mb-i];
		if(v > val)
		{
			val = v;
			idx = i;
...
void calc()
{
	...
	for(int i=1; i<MAXN; i++)
	{
		...
		reverse(ALL(vecs[i]));
		int sz = SZ(vecs[i]);
		for(int j=0; j<sz and j<ksize; j++)
			arr[j] = vecs[i][j];
		sum[0] = 0;
		for(int j=1; j<=sz and j<ksize; j++)
			sum[j] = sum[j-1] + arr[j-1];
		for(int j=sz+1; j<ksize; j++)
			sum[j] = sum[j-1];
