...
int sgn(pll a)
{
	return a.S > 0 or (a.S == 0 and a.F > 0);
}
...
int getNum(vector<ll> &v1, vector<ll> &v2, ll x)
{
	int s1 = SZ(v1);
	int s2 = SZ(v2);
	int ret = 0;
	...
	{
		ll v = v2[i];
//		while(pos < s1 and v <= x / v1[pos] and v * v1[pos] <= x)
		while(pos < s1 and (lll)v * v1[pos] <= (lll)x * x)
			pos++;
		ret += pos;
	}
...
int calc(int c)
{
	...
	for(auto &p: mp)
	{
		pll d1 = p.F;
		pll d2 = {-d1.S, d1.F};
		if(!mp.count(d2)) continue;
		vector<ll> &v1 = mp[d1], &v2 = mp[d2];
		ans += getNum(v1, v2, 2*B) - getNum(v1, v2, 2*A-1);
	}
